...
${this.generateTabs(5)}${randomChoicesBreakdown}`;
    ...
    getMapSalesforceFieldToFakerValue():Record<string, string> {
        ...
        const salesforceFieldToNPMFakerMap: Record<string, string> = {
            ...
            'number': `"\${{ faker.number.int({min: 0, max: 999999}) }}"`,
            'currency': `"\${{ faker.finance.amount(0, 999999, 2) }}"`,
            'percent': `"\${{ faker.number.float({ min: 0, max: 99, precision: 0.01 }).toFixed(2) }}"`,
            'date': `"\${{ faker.date.between({ from: new Date('2023-01-01'), to: new Date() }).toISOString().split('T')[0] }}"`,
            'datetime': `"\${{ faker.date.between({ from: new Date('2023-01-01T00:00:00Z'), to: new Date() }).toISOString() }}"`,
            'time': `"\${{ faker.date.between({ from: new Date('1970-01-01T00:00:00Z'), to: new Date('1970-01-01T23:59:59Z') }).toISOString().split('T')[1] }}"`,
