...
- **The Explorer bounds what it renders, and never re-labels what it drops** — an object's rows are built on first expand, and `applyModelLimits` caps objects, combinations per field and record type scopes per field. A row the check reported a failure for is never the thing dropped, and a dropped row is ABSENT and counted in a notice, never rendered as something it was not. Filtering works the same way: it hides, and recomputes no status. The three-state guarantee holds under both
