...
- **The panel answers a value query itself, from what it was already sent** — "what does `Canada` unlock" is a question about rows, and the tree is indexed by the DEPENDENT field, so one controlling field's effects sit in sibling nodes nothing composes. `renderControllingValueSummary` composes them, and the filter narrows to ROWS rather than stopping at nodes. Both are derived IN THE PANEL from `allowedValues` the model already carries, so the posted payload is unchanged — a test pins that no `searchText` in it names a dependent value. The distinction the row filter turns on needed a haystack that did not exist: since 3.18.0 `searchText` folds controlling values in with api names and so cannot say whether the reader named the NODE, which is what decides between showing all of a node's rows and narrowing it to one — hence `nodeNameSearchText`, names only, lowercased once at build time. What bounds the dependent-value index is its SOURCE: `declaredValues`, already deduplicated per field and already capped by `maxDeclaredValuesPerNode`, never `allowedValues`, which repeats the same picklist once per combination. Because `declaredValues` is a superset of every combination's `allowedValues`, a query it lacks cannot match any row — which is what makes it a sound PRE-FILTER rather than the match itself; a query it has still goes to the exact per-row rule, because the superset includes forbidden values no row unlocks. A row matches on what it UNLOCKS and never on its complement: every row that does not unlock a value forbids it, so matching the complement would show every row of the field. The index is built lazily, on the first query the posted haystack does not answer. MEASURE rather than reason if any of these caps change — at the combination ceiling with a 200-value picklist per field the index is 5.32 MB against a 39.8 MB payload, 35 ms once and 2.1 ms per keystroke, and two earlier drafts that indexed `allowedValues` per combination measured 84 MB and 91 MB, both chosen by reasoning and both wrong by an order of magnitude. And a miss states whether it is one: where the ceiling dropped rows or capped a universe, `0 of N` says the panel is not searching everything, and says it only when true
