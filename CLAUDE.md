...
└── treecipe/src/
    ...
    │   ├── RecipeCockpitService.ts           # Owns the Recipe Cockpit webview panel; loads a Generate Treecipe run (objects wrapper + recipe line numbers), pure shell/CSP/nonce builders and a pure message router
    │   ├── RecipeCockpitMetadataDiff.ts      # Pure recipe-vs-describe field classifier (new-in-org / removed-from-org / type-changed / picklist-changed / unchanged); imports nothing
    │   └── tests/
    │       └── mocks/                        # A GeneratedRecipes tree: two runs (snowfakery newer than faker-js), a run with no wrapper, a non-run folder; metadataDiff/ holds a recipe object set and normalized describes covering every diff category
    ...
    │   ├── SalesforceOrgService.ts           # Shared @salesforce/core connection helper, authorized-org picker, session-cached describe + pure describe normalization
    │   └── tests/
    │       └── mocks/                        # A describe result fixture
    ├── SfdxProjectService/
...
- **The find box is the FIRST thing the panel draws, and the ONLY thing above the rows is what is missing from them** — every block that used to sit here was a statement *about* the rows (where they came from, whether they still matched the metadata, what generated them, what the panel contains), and a reader who opened the panel to look one field up was scrolling past all of it to reach the only control that gets them there. All four are gone: the provenance banner, the contents, the scanned path and the generation stamp. What survives above the rows is the ceiling's truncation notices and the skipped-item count — the two caveats that say the panel is NOT SHOWING EVERYTHING, which a reader has to see before concluding a field has no dependency. `renderToolbar` runs first, and only when the model has objects: with none there is nothing to filter and `applyFilter`, which is what fills the match count, never runs. The skipped-item list is behind a disclosure and starts COLLAPSED — the COUNT is what a reader has to see, while the list is one line per skipped item and unbounded in how many the metadata skipped. Collapsed is not dropped: nothing leaves the model, and every warning still renders under its own object. Both header lines are out of the SHELL MARKUP, not merely left unwritten, and `revealHeaderLines` is gone with them: a model-filled header line written early is what made a failed render read as a finished one, and the guarantee is stronger when there is no such line to hold back. The contents went the same way (`renderTableOfContents`, `registerPanelSection`, `updatePanelSectionLabel`, `jumpToObject`), which makes the find box the whole navigation surface — a trade that costs a large org its per-object jump list. All of these are properties of the RENDERED PAGE rather than of the source, so they are asserted by running the real panel script against the fake DOM harness (`runPanelScript`) — which is also why that harness's `classList` answers `contains` from the classes an element actually carries rather than from its own `add` history: an element collapsed by `createElement('div', 'hidden')` was being reported visible, and a toggle driven off that answer opens a section the panel never closed — and why its fake elements START with the classes the shell markup declares, so a header line no render revealed is not reported visible either
- **The panel opens before the load and reports each phase; only what the structure does not depend on runs after the paint** — the shell is static, so it is shown immediately and names its phase in a banner and a status bar item. Reporting a phase is worth nothing without YIELDING: VS Code batches webview posts and status bar writes and flushes them at the end of the event-loop turn, so a load that never awaits narrates nothing however many phases it reports — `yieldToExtensionHost` between phases is what makes the reporting real, and it is also what lets the panel's `ready` be received mid-load. Every host message is stored and posted only once the panel is ready, which keeps the protocol idempotent: posting eagerly AND replaying on the handshake renders the whole model twice. The freshness check is GONE, not merely off the open path: the banner that held its button was its only entry point, so leaving the walk, the `checkFreshness` message, the host handler and `resolveManifestFreshness` in place would have left a subsystem nothing could reach. `buildSourceFingerprint` and `collectSourceFingerprintEntries` STAY — they are what a GENERATION records `sourceFingerprint` with, which is the half that was never the check; `manifest.json`'s schema and version are unchanged, and an existing manifest still loads. Nothing compares that fingerprint any more, and the panel makes no claim either way about whether the specs still match metadata. An open's last reported phase is `buildingView`. The run overlay that used to sit between the manifest parse and the model build is UNWIRED, and its `loadingResults` phase with it — an open now reports `readingManifest` then `buildingView`. The overlay itself is still there as `applyRunToViewModel`; nothing on the open path calls it. Before a model is rendered, every panel action allow-list is EMPTY — the panel exists in a window where nothing is on screen for an action to have come from
- **Nothing else can report a panel that failed to draw, so the panel reports itself** — a webview exception never reaches the extension host: a render that threw part way used to leave the host believing the load finished, because it had posted a model and the post had succeeded. The whole render runs inside `renderPanelGuarded`, which replaces the body with a failure notice (a partial page is an arbitrary prefix of the model, not a smaller correct answer) and posts `renderFailed` to `ErrorHandlingService.handleCapturedError` — the same path a host-side failure takes. A `rendered` ack distinguishes "a model was sent" from "something is on screen", and a `window` `error` listener covers throws outside the render, such as a lazy expand. There is no longer a model-filled header line to reveal at all: the scanned path used to be written FIRST, which made a failed render look like a finished one, and holding it back until the body had drawn was the fix. Deleting the line removes the failure mode outright rather than defending against it, which is why `revealHeaderLines` went with it and no part of this guard depends on it. Both panel→host messages are gated on a render message having been posted, like every other panel command. A failure to DRAW is distinguished from a `runtime` throw after a successful draw: only the first invalidates the panel and empties its action allow-lists, because a handler that threw on a keystroke leaves the rows on screen and readable. Each distinct failure is reported ONCE — the panel's `error` listener fires per event, and a throw in the filter's input handler would otherwise put one notification per keystroke in front of the reader
...
- **The Recipe Cockpit ships behind a workspace feature flag, and the COMMAND HANDLER is the gate** — every slice lands on main enabled by `salesforce-data-treecipe.recipeCockpitEnabled`, which `openRecipeCockpit` checks before a panel is created and which a modal warning is the only thing that writes. It is a runtime check rather than a `when` clause on the palette entry, because hiding a command hides it from the palette and from nothing else: a keybinding, a task or another extension can still execute it by id. Gating at the point the panel is created is also what saves every later slice a flag of its own — a cockpit nobody opted in to has no panel, hence no model built, no message posted and no rendered value for a panel action to have been derived from. The warning's link to the tracked work is a BUTTON (`View Known Issues`), because a VS Code dialog renders its detail as plain text and has no clickable link in it, and choosing it RE-SHOWS the warning rather than ending the command — a dialog closes on whichever button is clicked, so opening a browser would otherwise leave the reader to re-run the command to answer what they went to research. The url is a LABEL QUERY (`recipe-cockpit`) rather than issue numbers, which go stale as slices split, and it belongs to the dialog and never to the panel: the shell's `default-src 'none'` would render a link there dead, and interpolating any url into the template is the first step back toward a markup context the builder has none of. The flag is written at WORKSPACE scope, so opting in on one project says nothing about the next, and a dismissed warning writes nothing
- **A workspace-scoped config write can REJECT, and the answer is a boolean rather than a thenable nobody holds** — `setExtensionConfigValue` is `async` and returns whether the write landed, because VS Code refuses a `ConfigurationTarget.Workspace` write in a window with no folder open and an unawaited `update()` turns that into an unhandled rejection: nothing written, nothing reported, and the caller proceeding as though it had succeeded. That is invisible for a setting nothing reads back and the whole defect for one a user was just asked to choose. It reports the setting and the reason itself, for the call sites buried in paths with nothing to say to a user; the callers that CAN act on the answer do. Two do: `openRecipeCockpit` refuses outright when there is no workspace (the guard `openPicklistDependencyExplorer` already uses — a cockpit with no workspace has no recipe to traverse and no scope to record an opt-in at), and that guard comes BEFORE the flag check rather than after: the flag is WRITTEN at workspace scope but READ through the merged configuration (default < user < workspace) and is contributed in `package.json`, so a User-scope value would otherwise answer `true` in a folderless window and hand back a cockpit with nothing to render — a mutation test pins the order. It still opens the panel but says the workspace will ask again if the write fails for another reason; `activate` writes `useSnowfakeryAsDefault` only when a workspace folder exists, since warning a user about a setting they never chose is what writing unconditionally produced
- **The Recipe Cockpit's shell builder takes a nonce and nothing else, and that signature is the guarantee** — `buildWebviewShellHtml` interpolates the nonce and two compile-time constants in its own file, so the document carries no value this extension does not author. Recipes and org describes come from sources it does not control: they reach the panel over `postMessage` and are written through `textContent`, which is why the builder needs no escaping rather than having escaping that can be forgotten. A value from outside the file can only reach the template through a PARAMETER, so widening the signature to take a model is the single edit that re-opens the markup context — the same reason `localResourceRoots` is `[]` rather than omitted, which is what actually denies the grant (omitting it defaults to the extension directory plus every open workspace folder). `routePanelMessage(message, panelState)` is the whole host half of the protocol as a pure function returning an ACTION rather than a reply, so every allow-list decision is asserted without a webview and the side effects live in one executor; a command that carries a payload type-checks it and MATCHES it against an allow-list built from the rendered model, never validates it as a path. The panel is a singleton revealed rather than duplicated, its document rebuilt with a fresh nonce per load and its message subscription disposed before it is replaced — two listeners answering one `ready` is one acknowledgement the panel never asked for
- **The Recipe Cockpit traverses what Generate Treecipe WROTE, not a second derivation of it** — the objects wrapper (`treecipeObjectsWrapper-<ts>.json`) is the source of objects and fields, and the run's `.yml` files are the source of line numbers, read by the one layout `RecipeService` emits for BOTH backends (`- object:` at column zero, `  fields:`, one field per four-space line) — so there is no per-backend reader to drift. Runs are ordered by the timestamp SUFFIX, never the folder name: `recipe-fakerjs-` sorts above `recipe-` whatever the dates, and the fixture is shaped so that regression fails. Fields the recipe carries and the wrapper does not (standard-field mappings, the record type line — they never become a `FieldInfo`) are listed and marked `isOnlyInRecipeFile`, because a wrapper-only view says `Account` has no `Name`. A recipe path becomes something the host OPENS, so each passes `SfdxProjectService.isPathContainedInWorkspace` before it is kept; the model carries the path once per OBJECT and only a line number per field. Allow-lists have two generations: PENDING is built when a model is posted and only the panel's `rendered` — echoing that model's `renderSequence` — promotes it to ACTIVE, and every `ready` (each reveal reloads the document) empties ACTIVE — a post succeeding says nothing is on screen, and an ack of an OLDER model must not activate a newer one's targets. An object the wrapper holds with no `Fields` is a lookup target that `RelationshipService` still lists in `RecipeFiles.objects`; it is kept only if a recipe file carries its `- object:` line. A filter never HIDES an object: one with no match stays, collapsed and labelled, because a hidden one reads as "not in the recipe". Rows are built on first expand and a filter auto-expands at most `RECIPE_COCKPIT_AUTO_EXPAND_OBJECT_LIMIT` objects AND at most `RECIPE_COCKPIT_AUTO_EXPAND_ROW_BUDGET` rows (an expand builds every row, and an object can have 800 fields), so a keystroke's cost is bounded by what it expands rather than by what it matches. Containment is re-checked when a source is OPENED, not only when the model is built. There is no render ceiling: 12,000 fields posts 4.05 MB and 120,000 posts 40.67 MB (CHANGELOG 3.25.0) — re-measure when #57 adds per-row diff data
- **The Recipe Cockpit reaches an org only from inside the panel, and describes what the HOST rendered** — `selectOrg` carries no payload; the objects described are `describableObjectApiNames`, a pending/active allow-list like the others (promoted on `rendered`, emptied on every `ready`), captured BEFORE the picker opens, and a newly POSTED model empties the ACTIVE set too (not only the pending one) — otherwise a click between a new run's post and its ack describes the old run's objects under the new model's `renderSequence`. Object names come from workspace files and jsforce puts them in the describe URL PATH unencoded, so `describeObjects` refuses anything that is not an api name (`isUsableObjectApiName`) before any request, and the connection is opened by USERNAME so it names the org the cache key does. The answer is posted only if the model it described is still on screen, and the panel re-checks `renderSequence` — a describe of one run is never drawn over another's rows. Only a per-object SUMMARY is posted; the normalized field model lives in `SalesforceOrgService`'s session cache, keyed by org USERNAME (an alias can be re-pointed) and object, which a repeat request answers without connecting at all. Failures are never cached, and one object the org lacks is that object's outcome rather than the request's; a failed CONNECTION is the request's. `SalesforceOrgService.getConnection` is the one `Org.create` in the codebase — `CollectionsApiService.getConnectionFromAlias` delegates to it. `normalizeDescribeResult` is pure and type-checks everything, because the describe came over the network
- **The Recipe Cockpit's metadata diff imports NOTHING, and its inputs are structural** — `RecipeCockpitMetadataDiff` takes the cockpit's object view model and `SalesforceOrgService`'s normalized describe as they are, so there is no third field model, and a test fails if the file gains an import. Types are MAPPED (`DESCRIBE_TYPES_BY_METADATA_TYPE`) because the metadata and describe vocabularies differ (`Text`/`string`, `Lookup`/`reference`), and an empty or unmapped recipe type is not comparable rather than changed — as is any `__s` compound component, which `DirectoryProcessor` writes typed `Text` whatever it describes as. Org fields that are not createable are COUNTED rather than reported as new, and the org side is read as what the org HAS, so a failed describe must be left out by the caller, never passed as an absent object. Recipe picklist values reach it through `normalizeObjectsWrapper`'s host-only `picklistValuesByObjectApiName` and are kept off the posted field model; a DEPENDENT picklist records no values there, because one backed by a global value set carries only its `valueSettings` values and the wrapper cannot say which kind a field is
...
| `treecipe.openRecipeCockpit` | Open Recipe Cockpit |
