...
- **The Explorer panel owns its own look, and hard-codes none of it** — every colour in the shell stylesheet resolves through a semantic token (`--sdt-passed`, `--sdt-surface`, `--sdt-hairline`) to a `--vscode-*` token, and a test fails the build on a hex literal, an `rgb()` or a named colour anywhere in it: a literal colour survives a theme switch and removes the contrast a high contrast theme exists to provide. Status is carried by a SHAPE, a WORD and a colour in that order of reliability — three coloured dots collapse to one state for a reader who cannot separate the hues. The icons are an inline `<svg>` sprite drawn through `createElementNS`, deliberately NOT an icon font: the panel opens with an empty `localResourceRoots` and a `default-src 'none'` policy carrying no `font-src`, and a font would need both loosened. Dense mode COMPRESSES SPACE, NEVER INFORMATION — a test reads every `body.dense` rule and rejects `display: none`, because a density control that dropped rows would be a filter the reader never applied
- **What the reader was DOING is panel state; what the workspace HOLDS is host state** — the query, status filter, density and open objects live in the webview's own `setState`, because the panel is not retained when hidden and a reveal otherwise rebuilt the structure faithfully and the reader's place in it not at all. Expansions are keyed by API NAME rather than index (the model is rebuilt from the manifest in between, and an index reattaches to whichever object took that position), the restore is bounded by the same limit `Expand all` is, and state is rewritten on every render so names that no longer exist are pruned rather than accumulating forever. A focus filter is the opposite case: it belongs to one OPEN, so the host posts it after the render and stores nothing, which is what keeps it out of the reveal replay
...
| `treecipe.showFieldInPicklistDependencyExplorer` | Show in Picklist Dependency Explorer (context menu on `*.field-meta.xml`) |
